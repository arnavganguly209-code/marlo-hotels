...
export function buildBookingConfirmationEmailHtml(payload: BookingConfirmationEmailPayload) {
  ...
  const bookingUrl = payload.bookingUrl || `${siteConfig.url}/booking`;
  const pdfUrl = payload.pdfUrl || bookingUrl;
  ...
  <div style="max-width:640px;margin:auto;background:#fffdf8;border:1px solid #e4d6bd">
    <div style="padding:34px 42px;background:#0c1a18;color:#fffdf8;text-align:center">
      <div style="font-size:11px;letter-spacing:4px;color:#c9963f;text-transform:uppercase">Marlo Hotels</div>
      <div style="margin-top:12px;font-size:25px;letter-spacing:.5px">Stay Beyond Extraordinary</div>
    </div>
    <div style="padding:38px 42px">
      <p style="margin:0;color:#c9963f;font-size:11px;letter-spacing:2px;text-transform:uppercase">Reservation confirmed</p>
