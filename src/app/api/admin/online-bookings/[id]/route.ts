...
export async function PATCH(request: Request, { params }: Context) {
  ...
  const status = BOOKING_OPS_STATUSES.includes(body?.status as never) ? body?.status as string : existing.status;
  const requestedPayment = typeof body?.paymentStatus === "string" && body.paymentStatus === "UNPAID" ? "UNPAID" : body?.paymentStatus;
  const paymentStatus = PAYMENT_OPS_STATUSES.includes(requestedPayment as never) ? requestedPayment as string : existing.paymentStatus;
  const stringField = (key: string, fallback: string | null) => typeof body?.[key] === "string" ? body[key].trim() || null : fallback;
  ...
  return NextResponse.json({ booking: serialize(booking), email });
}
