...
export function BookingConfirmationDocument(booking: BookingConfirmationDocumentProps) {
  ...
  return (
    <main className="booking-confirmation-document">
      <header>
        <div className="logo-mark" aria-hidden>MH</div>
        <h1>MARLO HOTELS</h1>
      ...
      <DocumentSection title="Guest Information">{item("Name", booking.guestName)}{item("Email", booking.guestEmail)}{item("Phone", booking.guestPhone)}{item("Country", booking.country || "—")}{item("Special Requests", booking.notes || "—")}</DocumentSection>
      ...
      <DocumentSection title="Payment Information">{item("Room Rate", amount(booking.roomRate))}{item("Taxes", amount(booking.taxes))}{item("Additional Charges", amount(booking.additionalCharges))}{item("Grand Total", total)}{item("Payment Status", status(booking.paymentStatus))}{item("Payment Method", booking.paymentMethod || "Website / Pay on arrival")}</DocumentSection>
